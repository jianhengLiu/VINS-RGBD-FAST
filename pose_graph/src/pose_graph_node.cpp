...
queue<sensor_msgs::CompressedImageConstPtr> depth_buf;
...
void
image_callback(const sensor_msgs::CompressedImageConstPtr &image_msg, const sensor_msgs::CompressedImageConstPtr &depth_msg) {
...
void imu_forward_callback(const nav_msgs::Odometry::ConstPtr &forward_msg) {
    if (VISUALIZE_IMU_FORWARD) {
        Vector3d vio_t(forward_msg->pose.pose.position.x, forward_msg->pose.pose.position.y,
...
void process() {
    ...
    while (true) {
        ...
        if (pose_msg != NULL) {
            ...
            cv::Mat depth = cv::imdecode(imageData, cv::IMREAD_UNCHANGED);
...
//    SKIP_CNT = 0;
//    SKIP_DIS = 0;
//    std::string config_file = "/home/chrisliu/ROSws/vins_rgbd_ws/src/vins-rgbd-atlas/config/realsense/realsense_color_config.yaml";
    ...
    if (LOOP_CLOSURE) {
        ...
        // test param
//        std::string pkg_path = "/home/chrisliu/ROSws/vins_rgbd_ws/src/vins-rgbd-atlas/pose_graph";
    ...
    ros::Subscriber sub_pose = n.subscribe("/vins_estimator/keyframe_pose", 1000, pose_callback);
    //get extrinsic (ric qic  odometry.pose.pose.position and odometry.pose.pose.orientation)
    //update tic and qic real-time
    ros::Subscriber sub_extrinsic = n.subscribe("/vins_estimator/extrinsic", 1000, extrinsic_callback);
    //get keyframe_point(pointclude), store in point_buf (marginalization_flag == 0)
    ros::Subscriber sub_point = n.subscribe("/vins_estimator/keyframe_point", 1000, point_callback);


    // do relocalization here.
    // pose_graph publish match_points to vins_estimator, estimator then publish relo_relative_pose
    ros::Subscriber sub_relo_relative_pose = n.subscribe("/vins_estimator/relo_relative_pose", 1000,
    ...
    pub_match_points = n.advertise<sensor_msgs::PointCloud>("match_points", 1000);
