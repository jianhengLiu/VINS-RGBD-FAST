...
void registerPub(ros::NodeHandle &n) {
  pub_latest_odometry =
      n.advertise<nav_msgs::Odometry>("/vins_estimator/imu_propagate", 5);
  ...
  pub_imu_pose =
      n.advertise<geometry_msgs::PoseStamped>("/vins_estimator/imu_pose", 5);
  pub_key_poses =
      n.advertise<visualization_msgs::Marker>("/vins_estimator/key_poses", 5);
  pub_camera_pose =
      n.advertise<geometry_msgs::PoseStamped>("/vins_estimator/camera_pose", 5);
  pub_camera_pose_visual = n.advertise<visualization_msgs::MarkerArray>(
      "/vins_estimator/camera_pose_visual", 5);
  pub_extrinsic =
      n.advertise<nav_msgs::Odometry>("/vins_estimator/extrinsic", 5);
  ...
  pub_match = n.advertise<sensor_msgs::Image>(
      "/vins_estimator/feature_img", 1);
  pub_semantic = n.advertise<sensor_msgs::Image>(
      "/vins_estimator/semantic_mask", 1);

  pub_relo_path =
      n.advertise<nav_msgs::Path>("/vins_estimator/relocalization_path", 1);
  ...
  pub_keyframe_pose =
      n.advertise<nav_msgs::Odometry>("/vins_estimator/keyframe_pose", 1);
  pub_keyframe_point =
      n.advertise<sensor_msgs::PointCloud>("/vins_estimator/keyframe_point", 1);
  pub_point_cloud =
      n.advertise<sensor_msgs::PointCloud>("/vins_estimator/point_cloud", 1);
  pub_margin_cloud =
      n.advertise<sensor_msgs::PointCloud>("/vins_estimator/history_cloud", 1);
  pub_imu_pose =
      n.advertise<geometry_msgs::PoseStamped>("/vins_estimator/imu_pose", 5);
...
void printStatistics(const Estimator &estimator, double t) {
  ...
  Utility::R2ypr(estimator.ric[i]).transpose()); if (ESTIMATE_EXTRINSIC) {
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header) {
  ...
  if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR) {
    ...
    Vector3d delta_p =
        estimator.Ps[WINDOW_SIZE] - estimator.Ps[WINDOW_SIZE - 2];
    Vector3d delta_v =
...
void pubPointCloud(const Estimator &estimator, const std_msgs::Header &header) {
  ...
  for (auto &it_per_id : estimator.f_manager.feature) {
    ...
    // camera coordinate
    Vector3d pts_i = depth == 0 ? it_per_id.feature_per_frame[0].point *
...
void pubTF(const Estimator &estimator, const std_msgs::Header &header) {
  ...
  br.sendTransform(
      tf::StampedTransform(transform, header.stamp, "map", "body"));
...
void pubKeyframe(const Estimator &estimator) {
  ...
  if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR &&
    ...
    sensor_msgs::PointCloudPtr point_cloud_ptr(new sensor_msgs::PointCloud);
    point_cloud_ptr->header = estimator.Headers[WINDOW_SIZE - 2];
    for (auto &it_per_id : estimator.f_manager.feature) {
...
void pubTrackImg(const cv::Mat &img) {
  ...
  //    pub_match.publish(image_up);
  if (!img.empty())
    pub_match.publish(cv_bridge::CvImage(std_msgs::Header(), "bgr8", img)
                          .toImageMsg());
}
