...
void GlobalSFM::triangulateTwoFramesWithDepth(
  ...
  for (int j = 0; j < feature_num; j++) {
    ...
    for (int k = 0; k < (int)sfm_f[j].observation.size(); k++) {
      if (sfm_f[j].observation_depth[k].second < 0.1 ||
        ...
        continue;
      if (sfm_f[j].observation[k].first == frame0) {
        point0 = Vector3d(sfm_f[j].observation[k].second.x() *
                              sfm_f[j].observation_depth[k].second,
                          sfm_f[j].observation[k].second.y() *
                              sfm_f[j].observation_depth[k].second,
                          sfm_f[j].observation_depth[k].second);
        has_0 = true;
      }
...
bool GlobalSFM::construct(int frame_num, Quaterniond *q, Vector3d *T, int l,
  ...
  for (int i = l; i < frame_num - 1; i++) {
    ...
    triangulateTwoFramesWithDepth(i, Pose[i], frame_num - 1,
  ...
  for (int j = 0; j < feature_num; j++) {
    ...
    if ((int)sfm_f[j].observation.size() >= 2) {
      ...
      if (sfm_f[j].observation_depth[0].second < 0.1 ||
        ...
        continue;
      point0 = Vector3d(sfm_f[j].observation[0].second.x() *
                            sfm_f[j].observation_depth[0].second,
                        sfm_f[j].observation[0].second.y() *
                            sfm_f[j].observation_depth[0].second,
                        sfm_f[j].observation_depth[0].second);
      ...
      point_3d = Pose0_R.transpose() * point0 -
                 Pose0_R.transpose() * Pose0_t; // point in world;
      point1_reprojected = Pose1_R * point_3d + Pose1_t;
...
bool GlobalSFM::constructWithDepth(int frame_num, Quaterniond *q, Vector3d *T,
                                   int l, const Matrix3d relative_R,
