...
void RefineGravityWithDepth(map<double, ImageFrame> &all_image_frame,
 ...
 * @return      void
 */
bool LinearAlignmentWithDepth(map<double, ImageFrame> &all_image_frame,
                              Vector3d &g, VectorXd &x) {
  int all_frame_count = all_image_frame.size();
...
bool LinearAlignmentWithDepthGravity(map<double, ImageFrame> &all_image_frame,
                                     Vector3d &g, VectorXd &x) {
  int all_frame_count = all_image_frame.size();
  ...
  for (frame_i = all_image_frame.begin();
    ...
    //     frame_i->second.R.transpose() * dt * Matrix3d::Identity();
    tmp_b.block<3, 1>(3, 0) = frame_j->second.pre_integration->delta_v -
                              frame_i->second.R.transpose() * dt * g;
