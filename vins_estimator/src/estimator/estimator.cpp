...
void Estimator::optimization()
{
    ...
    for (int i = 0; i < frame_count + 1; i++)
    {
        ...
        if (find_solved[i] >= EST_FRAME && solver_flag == NON_LINEAR && marginalization_flag == MARGIN_SECOND_NEW)
        {
    ...
    if (relocalization_info)
    {
        ...
        for (auto &it_per_id : f_manager.feature)
        {
            if (it_per_id.is_dynamic || it_per_id.is_dynamic_mcc)
            {
                continue;
            }
            it_per_id.used_num = it_per_id.feature_per_frame.size();
...
{
    m_propagate.lock();
    latest_time = Headers[frame_count].stamp.toSec() + td;
...
Matrix3d Estimator::predictMotion(double t0, double t1)
{
    ...
    if (t1 <= imu_predict_buf.back().first)
    {
        ...
        while (imu_predict_buf.front().first <= t1 && !imu_predict_buf.empty())
        {
            ...
            relative_R *=
                AngleAxisd(cam0_angle_axisd.norm(), cam0_angle_axisd.normalized()).toRotationMatrix().transpose();
        }
    }
...
double Estimator::reprojectionError3D(Matrix3d &Ri, Vector3d &Pi, Matrix3d &rici, Vector3d &tici, Matrix3d &Rj,
                                      Vector3d &Pj, Matrix3d &ricj, Vector3d &ticj, double depth, Vector3d &uvi,
                                      Vector3d &uvj)
{
    Vector3d pts_w = Ri * (rici * (depth * uvi) + tici) + Pi;
...
void Estimator::movingConsistencyCheck(map<int, Eigen::Matrix<double, 7, 1>> &image)
{
    for (auto iter = image.begin(), iter_next = image.begin(); iter != image.end(); iter = iter_next)
    {
        ++iter_next;
        ...
        if (it != f_manager.feature.end())
        {
            ...
            double depth = it->feature_per_frame[0].depth;
            if (depth <= 0)
            {
                continue;
            }
            ...
            {
                double ave_err = err / errCnt;
                if (ave_err * FOCAL_LENGTH > 20)
                {
                    image.erase(iter);
                    it->is_dynamic_mcc = true;
                    // cout << "ave_err * FOCAL_LENGTH = " << ave_err * FOCAL_LENGTH << endl;
                }
                else if (ave_err * FOCAL_LENGTH < 3)
                {
                    it->is_dynamic_mcc = false;
                }
            }
        }
    }
